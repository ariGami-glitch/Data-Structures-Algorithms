...
public class BinarySearchTree<Type extends Comparable<Type>> {
    ...
    /**
     * This method calls a helper method to count the number of nodes in the binary search tree
     *
     * @return an int which represents the number of nodes
     */
    public int count() {
        //call the helper method to get the number of nodes
        int numNodes = countHelper(root);
        //return the number of nodes
        return numNodes;
    }

    /**
     * This method is the helper method that recursively call itself to count the number of nodes
     * @param node which is a node that represents the node being called on to count
     * @return an int which is the number of nodes counted so far
     */
    private int countHelper(Node node) {
        ...
        if(node == null) {
            ...
            //recursively call this method to count the number of nodes
            return countHelper(node.right) + countHelper(node.left) + 1;
        }
